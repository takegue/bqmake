create or replace function
    `v0.zbqt_gensql__table_profiler`(
        target_table_name string, group_keys array<string>, options_json json
    )
as
    (
        (
            with
                options as (
                    select
                        ifnull(
                            bool(options_json.materialized_view_mode), false
                        ) as option_materialized_view_mode,
                        ifnull(
                            int64(options_json.numeric_precision), 6
                        ) as option_numeric_precision
                ),
                table_columns as (
                    select
                        table_catalog,
                        ...
                        depth,
                        max(partition_column) over (
                            partition by table_name
                        ) as partition_column,
                        row_number() over (
                            partition by
                                table_name, column_name, ordinal_position, depth
                        ) as subposition,
                        path.data_type,
                        starts_with(c.data_type, 'ARRAY') as is_under_array
                    from `bqtest.INFORMATION_SCHEMA.COLUMNS` c
                    left join
                        `bqtest.INFORMATION_SCHEMA.COLUMN_FIELD_PATHS` as path using (
                            table_catalog, table_schema, table_name, column_name
                        )
                    -- For expanding views
                    left join unnest([struct(target_table_name as target_table)])
                    left join
                        unnest(
                            [
                                struct(
                                    array_length(
                                        regexp_extract_all(field_path, r'\.')
                                    ) as depth,
                    ...
                    where
                        if(
                            has_wildcard,
                            starts_with(
                                table_name, regexp_replace(target_table, r'\*$', '')
                            ),
                            table_name = target_table
                        )
                    qualify
                        1 = row_number() over (
                            partition by table_schema, target_table, field_path
    ...
    """
      ...
      """, 0),
            ...
            """,
                    ...
                    left join
                        `bqtest.INFORMATION_SCHEMA.VIEWS` as path using (
      ...
      """ as advanced, "" as subprofile) as string,
                                    struct(r"""
        , countif(!fieldname! is not null) as !column!__nonnull
        , hll_count.init(string(date(!fieldname!))) as !column!__day_hll
        , min(!fieldname!) as !column!__min
        , max(!fieldname!) as !column!__max
      """ as restricted, "" as advanced, "" as subprofile) as timestamp,
                                    struct(r"""
        -- !column! (!fieldnum!)
        , countif(!fieldname! is not null) as !column!__nonnull
      """ as restricted, "" as advanced, "" as subprofile) as anything
                                )
                            ]
                        ) as template
                    left join
            ...
            select any_value(`bqmake.v0.zdeindent`(ddl.query))
            from ddl
...
execute immediate format("with validateSQL as (%s) select 1", `v0.zbqt_gensql__table_profiler`("demo_sample_table", null, null));
...
execute immediate format("with validateSQL as (%s) select 1", `v0.zbqt_gensql__table_profiler`("demo_sample_table", ["status"], null));
...
execute immediate format("with validateSQL as (%s) select 1", `v0.zbqt_gensql__table_profiler`("demo_sample_partition_table", null, null));
execute immediate 'create materialized view `bqtest.mateview1` options(enable_refresh = false) as \n'
  || `v0.zbqt_gensql__table_profiler`(
...
execute immediate format("with validateSQL as (%s) select 1", `v0.zbqt_gensql__table_profiler`("demo_sample_view", [], null));
execute immediate 'create materialized view `bqtest.mateview1` options(enable_refresh = false) as \n'
  || `v0.zbqt_gensql__table_profiler`(
    "demo_sample_view"
    , null

        ...
        (
            with
                views as (
                    ...
                    union all
                    select format(ltrim(`v0.zdeindent`("""
      # Auto-generated SQL by bqmake.bqtest
      select * from `%s.%s.%s`
    """)), table_catalog, table_schema, table_name)
                    from `bqtest.INFORMATION_SCHEMA.TABLES`
                    where table_name = _table_name and table_type = 'BASE TABLE'
                ),
                switched as (
                    select
                        view_definition,
      ...
      """,
                                trim(
                                    substr(
                                        view_definition,
                                        `bqtest.zfind_final_select`(view_definition) + 1
                                    )
                                )
                            )
                        )
                        ...
                        || array_to_string(
                            array(
                                ...
                                left join
                                    unnest(
                                        `bqtest.zfind_ctes`(view_definition)
                                        || [string(null)]
                                    ) as cte
...
begin
  execute immediate `v0.zbqt_gensql__table_spec`(
    "demo_sample_table"
    , [

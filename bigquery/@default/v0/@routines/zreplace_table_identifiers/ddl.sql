create or replace function
    `v0.zreplace_table_identifiers`(
        sql string, replacements array<struct<from_value string, to_value string>>
    )
returns string
language js
as r"""
...

  {
    ...
    for (const { token, left, right, type } of tokens) {
      ...
      if (type === "TABLE_IDENTIFIER") {
        for (const { from_value, to_value } of replacements) {
          text = text.replace(from_value, to_value);
        }
      }
      ret.push(text);
      lastWrote = right + 1;
...
return replace_identifier(sql, replacements)
"""
;

begin
select `v0.zreplace_table_identifiers`(input, [("cte1", "hoge")])
from unnest([struct(r"""

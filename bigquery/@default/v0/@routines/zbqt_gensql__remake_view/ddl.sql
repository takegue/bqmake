            ...
            with
                ...
                switched as (
                    select
                        view_definition,
                        rtrim(
                            left(remaked, `bqmake.bqtest.zfind_final_select`(remaked))
                        ) || format(
                    ...
                    left join
                        unnest(
                            [
                                struct(
                                    `bqmake.bqtest.zreplace_table_identifiers`(
                                        view_definition, _mocking_ctes
                                    ) as remaked
                                )
                            ]
                        )
...
begin
with deps as (select * from bqtest.demo_sample_view)
select 1
;

call `bqmake.v0.assert_golden`(
    (null, "bqtest", "zgolden_routines"),  -- Profiling query
    `bqtest.zbqt_gensql__udf_snapshot`([`bqmake.v0.zreindent`("""
...
;
  ...
  execute immediate `v0.zbqt_gensql__remake_view`(
    'demo_sample_view', '__test_count', [('datasource', 'datasource_sampled')]
  );
end

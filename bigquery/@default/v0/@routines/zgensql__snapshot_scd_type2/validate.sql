...
set destination = (null, "sandbox", "stations_scd_type2");

drop table if exists `sandbox.stations_scd_type2`;
...
for t in (select as struct
    ...
    from unnest([
      ...
      , ("Almost all record is updated", 101, 'select * replace(if(station_id in (2499), status, "closed") as status) from `bigquery-public-data.austin_bikeshare.bikeshare_stations`')
...
do
  ...
  select if(
    ...
    , format("Passed: %s", t.msg)
    , error(format('Failed: %s (%t != %t)', t.msg, t.expected_row_count, @@row_count))
  );
end for;
